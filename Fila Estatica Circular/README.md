...
Implementação de fila estática circular em C com operações básicas de inserção, remoção, consulta e exibição
...
├── include/
|   └── filaEstaticaCircular.h      # Declarações de estruturas e funções (.h)
├── src/
|   └── filaEstaticaCircular.c      # Implementação das funções/operações (.c)
...
```bash
cd ".\Estrutura-de-Dados-em-C\Fila Estatica Circular"
```

3. Compile o código :

```bash
gcc -I./include main.c src/filaEstaticaCircular.c -o filaEstaticaCircular
```

4. Execute o programa :

```bash
filaEstaticaCircular.exe
```
