...
Implementação de lista dinâmica circular em C com operações básicas de inserção, remoção, busca, alteração e exibição

## 📁 Estruturas do Projeto

```
Lista Dinamica Circular/
├── include/
|   └── listaDinamicaCircular.h     # Declarações de estruturas e funções (.h)
├── src/
|   └── listaDinamicaCircular.c     # Implementação das funções/operações (.c)
...
| `exibir()` | Exibir um único elemento da lista |
...
- Operações em estrutura de dados (inserção, remoção, busca, alteração);
...
```bash
cd ".\Estrutura-de-Dados-em-C\Lista Dinamica Circular"
```

3. Compile o código :

```bash
gcc -I./include main.c src/listaDinamicaCircular.c -o listaDinamicaCircular
```

4. Execute o programa :

```bash
listaDinamicaCircular.exe
```
